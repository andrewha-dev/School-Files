...
public class BasicCollectionTest
{
	...
	{
		return new junit.framework.JUnit4TestAdapter(BasicCollectionTest.class);
	}
	...
	public void testRetainAll()
	{
		...
		c.add("cat");
		c.add("two");
		assertEquals("c size before retainAll", 7, c.size());
		Collection<String> c1 = new BasicCollection<String>();
		c1.add("dog");
		c1.add("Bird");
		c1.add("cat");
		c1.add("two");
		assertTrue("retainAll successful", c.retainAll(c1));
		assertEquals("c size after retainAll", 5, c.size());
		for (String d : c1)
			assertTrue("retainAll c does contain c1"+d, c.contains(d));
